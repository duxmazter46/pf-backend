...
app.put("/todo", async (req, res, next) => {
  try {
    ...
    const result = await dbClient
      ...
      .returning({ id: todoTable.id, todoText: todoTable.todoText, tag: todoTable.tag, rank: todoTable.rank });
...
app.patch("/todo", async (req, res, next) => {
  try {
    ...
    const updateValues: Partial<{ todoText: string, tag: string, rank: number }> = { todoText };
    if (tag !== undefined) updateValues.tag = tag;
    if (rank !== undefined) updateValues.rank = rank;
